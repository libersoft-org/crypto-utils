...
import { provider, getProviderUrl } from "./provider.ts";
 ...
 */
export const tokenInfos = writable<Map<ContractAddress, ITokenLoadedInfo>>(
	new Map(),
);
 ...
 */
export const tokenBalances = writable<Map<ContractAddress, IBalanceWithFiat>>(
	new Map(),
);
...
export const tokensForDisplay = derived(
	...
	([
		...
		const tokensWithContracts = $tokenConfs.filter(
			(token) =>
				token.contract_address &&
				isValidContractAddress(token.contract_address),
		);

		// Transform into display-ready data
		return tokensWithContracts
			.map((t) => {
...
export async function refreshTokenBalance(
	...
	loadingTokens.update((set) =>
		updateReactiveSet(set, (s) => s.add(contractAddress)),
	);
	...
	try {
		...
		if (amount) {
			...
			tokenBalances.update((map) =>
				updateReactiveMap(map, (m) => {
		...
		console.error(
			`Error refreshing token balance for ${contractAddress}:`,
			error,
		);
	} finally {
		loadingTokens.update((set) =>
			updateReactiveSet(set, (s) => s.delete(contractAddress)),
		);
	}
}
...
export async function loadAllTokenInfos(): Promise<void> {
	...
	const tokensToLoad = tokensWithContracts.filter(
		(token) =>
			token.contract_address &&
	...
	// Mark all as loading
	loadingTokenInfos.update((set) =>
		updateReactiveSet(set, (s) => {
			tokensToLoad.forEach(
				(token) => token.contract_address && s.add(token.contract_address),
			);
		}),
	);

	try {
		const contractAddresses = tokensToLoad.map(
		...
		loadingTokenInfos.update((set) =>
			updateReactiveSet(set, (s) => {
				tokensToLoad.forEach(
					(token) => token.contract_address && s.delete(token.contract_address),
				);
			}),
		);
	}
...
async function fallbackBatchBalanceCallByAddress(
	tokensWithAddresses: any[],
	provider: any,
	network: any,
	addr: any,
): Promise<Map<string, IBalance>> {
	...
	try {
		...
		console.log(
			`Fallback: Sending JSON-RPC batch with ${batchPayload.length} calls for ${tokensWithAddresses.length} token balances`,
		);
		// Get provider URL
		...
		// Process results
		const contract = new Contract(
			tokensWithAddresses[0].contract_address,
			erc20BalanceABI,
			provider,
		);
		for (let i = 0; i < tokensWithAddresses.length; i++) {
			...
			try {
				...
				if (balanceResult?.result && decimalsResult?.result) {
					...
					const decimals = contract.interface.decodeFunctionResult(
						"decimals",
					...
					console.debug(
						`Failed to get balance for token ${token.contract_address} via JSON-RPC batch`,
					);
				}
			} catch (error) {
				console.debug(
					`Error processing token ${token.contract_address} from JSON-RPC batch:`,
					error instanceof Error ? error.message : String(error),
				);
			}
		}
		console.log(
			`JSON-RPC batch processed ${result.size}/${tokensWithAddresses.length} token balances successfully`,
		);
	} catch (error) {
...
async function tryMulticallBalancesByAddress(
	tokensWithAddresses: any[],
	provider: any,
	network: any,
	addr: any,
): Promise<Map<string, IAmount> | null> {
	return executeMulticallBalancesByAddress(
		tokensWithAddresses,
		provider,
		network,
		addr,
...
async function executeMulticallBalancesByAddress(
	tokensWithAddresses: any[],
	provider: any,
	_network: any,
	addr: any,
	...
	try {
		console.log(
			`Trying Multicall3 for ${tokensWithAddresses.length} token balances by address`,
		);
		...
		console.log(
			`ccUsing Multicall for ${tokensWithAddresses.length} token balances (${calls.length} calls) in ONE blockchain transaction`,
		);
		// Execute Multicall
		...
		for (let i = 0; i < tokensWithAddresses.length; i++) {
			...
			try {
				if (returnData[balanceIndex] && returnData[decimalsIndex]) {
					...
					console.debug(
						`Failed to get balance for token ${token.contract_address} via Multicall`,
					);
				}
			} catch (error) {
				console.debug(
					`Error processing token ${token.contract_address} from Multicall:`,
					error instanceof Error ? error.message : String(error),
				);
			}
		}
		console.debug(
			`Multicall processed ${result.size}/${tokensWithAddresses.length} token balances successfully`,
		);
		return result;
	} catch (error) {
...
export async function getBatchTokenAmountsByAddresses(
	contractAddresses: string[],
): Promise<Map<string, IAmount>> {
	...
	if (!net || !p || !addr || contractAddresses.length === 0) {
		console.error(
			"Network, provider, address not set or no addresses provided",
		);
		return result;
	}

	// Create token objects with addresses for compatibility with existing functions
	const tokensWithAddresses = contractAddresses.map((address) => ({
		contract_address: address,
	}));

	console.log(
		`Starting batch balance loading for ${tokensWithAddresses.length} tokens by addresses`,
	);
	...
	try {
		...
		if (remainingTokens.length > 0) {
			const multicallResult = await tryMulticallBalancesByAddress(
				remainingTokens,
				p,
				net,
				addr,
			...
			if (multicallResult && multicallResult.size > 0) {
				...
				remainingTokens = remainingTokens.filter(
					(token) => !multicallResult.has(token.contract_address),
				);
				console.log(
					`Multicall succeeded for ${multicallResult.size} tokens, ${remainingTokens.length} tokens remaining`,
				);
			} else {
				console.log(
					"Multicall returned no results, all tokens will try fallback",
				);
			}
		}
		...
		if (remainingTokens.length > 0) {
			try {
				const batchResult = await fallbackBatchBalanceCallByAddress(
					remainingTokens,
					p,
					net,
					addr,
				...
				if (batchResult.size > 0) {
					...
					remainingTokens = remainingTokens.filter(
						(token) => !batchResult.has(token.contract_address),
					);
					console.log(
						`JSON-RPC batch succeeded for ${batchResult.size} tokens, ${remainingTokens.length} tokens still remaining`,
					);
				}
			} catch (error) {
				console.debug(
					"JSON-RPC batch for balances failed, falling back to individual calls:",
					error,
				);
			}
		}
		...
		if (remainingTokens.length > 0) {
			console.log(
				`Fallback: Loading ${remainingTokens.length} token balances individually`,
			);
			const balancePromises = remainingTokens.map(async (token) => {
				try {
					const tokenBalance = await getTokenBalanceByAddress(
						token.contract_address,
					);
					return {
	...
	console.log(
		`Final result: ${result.size}/${contractAddresses.length} token balances loaded successfully`,
	);
	return result;
}

export async function getTokenBalanceByAddress(
	...
	try {
		...
		const [balance, decimals, symbol] = await Promise.all([
			contract["balanceOf"]!(addr.address),
			contract["decimals"]!(),
			contract["symbol"]!(),
		]);
		return {
		...
		console.debug(
			"Error while getting token balance by address:",
			error instanceof Error ? error.message : String(error),
		);
		return null;
	}
...
async function getDirectTokenBalance(
	token: any,
	provider: any,
	addr: any,
): Promise<IBalance | null> {
...
export async function getBatchTokenBalances(): Promise<Map<string, IBalance>> {
	...
	const tokensWithAddresses = tokenList.filter(
		(token) =>
			token.contract_address && isValidContractAddress(token.contract_address),
	);
	...
	try {
		// Try Multicall3 first
		if (remainingTokens.length > 0) {
			const multicallResult = await tryMulticallBalances(
				remainingTokens,
				p,
				net,
				addr,
			...
			if (multicallResult && multicallResult.size > 0) {
				...
				console.log(
					`Multicall returned no results, all ${remainingTokens.length} tokens will try fallback`,
				);
			}
		}
		...
		if (remainingTokens.length > 0) {
			try {
				const batchResult = await fallbackBatchBalanceCall(
					remainingTokens,
					p,
					net,
					addr,
	...
	console.log(
		`Final result: successfully loaded ${result.size}/${tokensWithAddresses.length} token balances`,
	);
	console.log("Successful tokens:", Array.from(result.keys()));
	const failedTokens = tokensWithAddresses.filter(
		(token) => !result.has(token.contract_address),
	);
...
async function tryMulticallBalances(
	tokensWithAddresses: any[],
	provider: any,
	network: any,
	addr: any,
): Promise<Map<string, IBalance> | null> {
...
async function executeMulticallBalances(
	tokensWithAddresses: any[],
	provider: any,
	network: any,
	addr: any,
	...
	try {
		console.debug(
			`Trying Multicall3 at ${multicall3Address} for chainID ${network.chainID}`,
		);
		...
		console.debug(
			`bbUsing Multicall for ${tokensWithAddresses.length} token balances (${calls.length} calls) in ONE blockchain transaction`,
		);
		...
		// Process results
		return processTokenBalanceResults(
			returnData,
			tokensWithAddresses,
			erc20Interface,
			[],
...
function processTokenInfoBatchResults(
	batchResults: any[],
	addresses: string[],
	contract: Contract,
): Map<
	...
	for (let i = 0; i < addresses.length; i++) {
		...
		try {
			...
			if (nameResult?.result && symbolResult?.result) {
				const name = contract.interface.decodeFunctionResult(
					"name",
					nameResult.result,
				)[0];
				const symbol = contract.interface.decodeFunctionResult(
					"symbol",
					symbolResult.result,
				)[0];
				result.set(address, {
...
async function fallbackBatchCall(
	contractAddresses: string[],
	provider: any,
	network: any,
): Promise<
...
> {
	return executeBatchCall(
		...
		network,
		[],
		processTokenInfoBatchResults,
	);
}

// Process token balance results from JSON-RPC batch
function processTokenBalanceBatchResults(
	batchResults: any[],
	tokens: any[],
	contract: Contract,
): Map<string, IBalance> {
	...
	for (let i = 0; i < tokens.length; i++) {
		...
		try {
			...
			if (balanceResult?.result && decimalsResult?.result) {
				...
				console.log(
					`Successfully got balance for ${token.symbol} via JSON-RPC batch`,
				);
			} else {
				console.warn(
					`Failed to get balance for token ${token.symbol} via JSON-RPC batch - empty result`,
				);
			}
		} catch (error) {
			console.info(
				`Error processing token balance ${token.symbol} from JSON-RPC batch:`,
				error instanceof Error ? error.message : String(error),
			);
		}
	}
	...
	console.log(
		`JSON-RPC batch processed ${result.size}/${tokens.length} token balances successfully`,
	);
	return result;
}

// Special JSON-RPC batch executor for balances (handles different function parameters)
async function executeBatchBalanceCall(
	tokensWithAddresses: any[],
	provider: any,
	network: any,
	addr: any,
	...
	try {
		...
		return processTokenBalanceBatchResults(
			batchResults,
			tokensWithAddresses,
			contract,
		);
...
async function fallbackBatchBalanceCall(
	tokensWithAddresses: any[],
	provider: any,
	network: any,
	addr: any,
...
function processTokenBalanceResults(
	...
	for (let i = 0; i < tokens.length; i++) {
		...
		try {
			if (returnData[balanceIndex] && returnData[decimalsIndex]) {
				...
				console.log(
					`Successfully got balance for ${token.symbol} via Multicall`,
				);
			} else {
				console.warn(
					`Failed to get balance for token ${token.symbol} via Multicall - empty response`,
				);
			}
		} catch (error) {
			console.info(
				`Error processing token balance ${token.symbol} from Multicall:`,
				error instanceof Error ? error.message : String(error),
			);
		}
	}
	...
	console.log(
		`Multicall processed ${result.size}/${tokens.length} token balances successfully`,
	);
	return result;
}
...
> | null> {
	return executeMulticall(
		...
		network,
		[],
		processTokenInfoResults,
	);
}

// Generic JSON-RPC batch executor
async function executeBatchCall<T>(
	addresses: string[],
	abi: string[],
	...
	try {
		...
		console.log(
			`Fallback: Sending JSON-RPC batch with ${batchPayload.length} calls for ${addresses.length} tokens`,
		);
		...
		if (!firstAddr) return result;
		const contract = new Contract(firstAddr, abi, provider); // For decoding
...
function processTokenInfoResults(
	returnData: string[],
	addresses: string[],
	erc20Interface: any,
): Map<
...
> {
	...
	for (let i = 0; i < addresses.length; i++) {
		...
		try {
			...
			console.info(
				`Error processing token ${address} from Multicall:`,
				error instanceof Error ? error.message : String(error),
			);
		}
	}
 ...
 */
export async function getBatchTokensInfo(
	contractAddresses: ContractAddress[],
): Promise<Map<ContractAddress, ITokenLoadedInfo>> {
	const p = get(provider);
	const net = get(selectedNetwork);
	const result = new Map<string, { name: string; symbol: string }>();
	...
	try {
		...
		if (multicallResult && multicallResult.size > 0) {
			...
			const missingAddresses = contractAddresses.filter(
				(address) => !multicallResult.has(address),
			);
			if (missingAddresses.length === 0) return result; // All addresses successful
			console.log(
				`Multicall info succeeded for ${multicallResult.size} tokens, trying fallback for ${missingAddresses.length} missing tokens`,
			);
