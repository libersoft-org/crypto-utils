...
export function checkWebHIDSupport(): { supported: boolean; error?: string } {
	...
	if (!("hid" in navigator)) {
		...
		return {
			supported: false,
			error:
				"WebHID is not supported in this browser. Please use Chrome/Edge 89+ or another compatible browser.",
		};
	}
...
export function checkWebUSBSupport(): { supported: boolean; error?: string } {
	...
	if (!("usb" in navigator)) {
		...
		return {
			supported: false,
			error:
				"WebUSB is not supported in this browser. Please use Chrome/Edge 89+ or another compatible browser.",
		};
	}
 ...
 */
export function checkConnectionMethodSupport(method: "WebHID" | "WebUSB"): {
	supported: boolean;
	error?: string;
} {
...
export async function initializeLedger(): Promise<boolean> {
	...
	try {
		...
		if (!webHIDSupport.supported && !webUSBSupport?.supported) {
			const errorMsg =
				"Neither WebHID nor WebUSB is supported in this browser. Please use Chrome/Edge 89+ or another compatible browser.";
		...
		const errorMessage =
			error instanceof Error ? error.message : "Failed to initialize Ledger";
	...
	try {
		...
		try {
			if (connectionMethod === "WebHID") {
				...
				console.log(
					"WebUSB device listener not available - using basic connection",
				);
			}
		} catch (listenerError) {
			console.warn(
				"Could not set up device listener (non-critical):",
				listenerError,
			);
		...
		if (error instanceof Error) {
			...
			if (error.name === "NotAllowedError") {
				errorMessage =
					"Connection denied. Please make sure you:\n" +
				...
				errorMessage =
					"Device is busy. Please close Ledger Live and try again.";
			} else if (
				error.message.includes("CLA_NOT_SUPPORTED") ||
				error.message.includes("UNKNOWN_APDU") ||
				...
				errorMessage =
					"App not ready. Please wait for the Ethereum app to fully load.";
			} else {
 ...
 */
export async function getLedgerEthereumAccounts(
	startIndex: number = 0,
	count: number = 5,
...
export async function signEthereumTransaction(
...
): Promise<LedgerResponse<{ r: string; s: string; v: number }>> {
	if (!currentTransport)
		return {
			success: false,
			payload: null as any,
	...
	try {
		...
		const to = transaction.to
			? String(await Promise.resolve(transaction.to))
			: undefined;
		...
		if (isEIP1559 || transaction.type === 2) {
			...
			console.log(
				"Creating legacy transaction without chainId to avoid Type 1 serialization",
			);
		}
		...
		try {
			...
			const txHexForLedger = unsignedTx.startsWith("0x")
				? unsignedTx.slice(2)
				: unsignedTx;
			...
			console.warn(
				"Failed to resolve transaction, falling back to blind signing:",
				resolveError,
			);
		...
		const txHexForSigning = unsignedTx.startsWith("0x")
			? unsignedTx.slice(2)
			: unsignedTx;
		...
		if (
			errorMessage.includes("EthAppPleaseEnableContractData") ||
			errorMessage.includes("Please enable Blind signing or Contract data")
		) {
			errorMessage =
				'Token transactions require "Contract data" to be enabled on your Ledger device.\n\n' +
				"To enable:\n" +
 ...
 */
export async function getLedgerEthereumPublicKey(
	path: string,
): Promise<LedgerResponse<{ publicKey: string; address: string }>> {
	if (!currentTransport)
		return {
			success: false,
			payload: null as any,
 ...
 */
export async function doAddHardwareAddressLedger(
	_w: any,
	addresses: any[],
	index: number,
	name?: string,
