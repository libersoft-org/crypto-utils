...
import type { Device } from "@trezor/connect/lib/types";
import TrezorConnectWeb, {
	DEVICE_EVENT as DEVICE_EVENT_WEB,
} from "@trezor/connect-web";
import TrezorConnectNode, {
	DEVICE_EVENT as DEVICE_EVENT_NODE,
} from "@trezor/connect";

// Use the appropriate Trezor Connect based on environment
const TrezorConnect =
	typeof window !== "undefined" ? TrezorConnectWeb : TrezorConnectNode;
const DEVICE_EVENT =
	typeof window !== "undefined" ? DEVICE_EVENT_WEB : DEVICE_EVENT_NODE;
...
export const usePopup = localStorageSharedStore<boolean>(
	"trezorUsePopup",
	false,
);

export const trezorDevice = derived(
	[trezorState, trezorDevices],
	([$trezorState, $trezorDevices]) => {
		if (!$trezorState || !$trezorState.device) {
			return null;
		}
		const deviceId = $trezorState.device.state.staticSessionId
			.split(":")[0]
			.split("@")[1];
...
export const isHwWalletActive = (wallet?: IWallet): boolean => {
	...
	return (
		!!$staticSessionId &&
		$staticSessionId === wallet?.identifiers?.staticSessionId
	);
};
...
function isDeviceConnectEvent(
	event: any,
): event is { type: "device-connect"; payload: Device & { id: string } } {
	return event.type === "device-connect";
}

// Type guard for device disconnect event
function isDeviceDisconnectEvent(
	event: any,
): event is { type: "device-disconnect" } {
	return event.type === "device-disconnect";
}

// Helper function to manage loading/error state for async operations
export async function withTrezorState<T>(
	operation: () => Promise<T>,
): Promise<T> {
...
export async function getTrezorEthereumAccounts(
	startIndex: number = 0,
	count: number = 1,
	w?: IWallet,
): Promise<TrezorAccount[]> {
...
export async function doAddHardwareAddressTrezor(
	w: IWallet,
	addresses: IAddress[],
	index: number,
	name?: string,
