...
export interface INftDef {
	...
	token_id?: string | undefined; // Optional specific token ID for ERC-1155
}
...
// Stores
export const nftCollectionInfos = writable<
	Map<ContractAddress, INftCollectionInfo | null>
>(new Map());
...
export const nftsForDisplay = derived(
	...
	([
		...
		return $nftConfs.map((conf) => {
			...
			const isLoadingCollection = $loadingNftCollections.has(
			...
			return {
				...
				isLoadingCollection:
					isLoadingCollection ||
					!$wasEverLoadingNftCollections.has(conf.contract_address),
				isLoadingToken:
					isLoadingToken || !$wasEverLoadingNftTokens.has(conf.guid),
				isLoadingBalance:
					isLoadingBalance || !$wasEverLoadingNftBalances.has(conf.guid),

				displayName:
					(collectionInfo?.name || "NFT") +
					(conf.token_id ? ` #${conf.token_id}` : "") +
...
async function executeNftOperation<T>(
	...
	const standard = await detectNftStandard(
		configuredNft.contract_address,
		provider,
	);
	...
	try {
		...
		console.debug(
			`Failed NFT operation for ${standard} contract ${configuredNft.contract_address}:`,
 ...
 */
export async function loadNFTCollectionInfos(
	...
	const contractsToLoad = contractAddresses.filter(
		(addr) => !currentlyLoading.has(addr) && !currentInfos.has(addr),
	);

	if (!contractsToLoad.length) return;

	console.log(
		"Loading NFT collection infos for",
		contractsToLoad.length,
		"contracts",
	);
	...
	nftCollectionInfos.update((map) => {
		results.forEach((result, index) => {
			...
			if (result.status === "fulfilled") {
				...
				console.warn(
					`Failed to load collection info for ${contractAddress}:`,
					result.reason,
				);
...
export async function enumerateOwnedErc721Tokens(
	...
	try {
		...
		console.warn(
			`Contract ${contractAddress} doesn't support token enumeration`,
		);
		return null;
	}
}
 ...
 */
function createNftMetadataObject(
	metadata: any,
	_tokenId: string,
	defaultName: string,
): INftLoadedInfo {
 ...
 */
async function loadErc721Metadata(
	configuredNft: INftConf,
	provider: any,
	_address: string,
): Promise<INftLoadedInfo | null> {
	// ERC721 tracking requires specific token ID
	if (!configuredNft.token_id) {
		console.warn(
			`loadErc721Metadata: ERC721 NFT ${configuredNft.contract_address} requires token_id to be specified`,
		);
		return null;
	}
	...
	return createNftMetadataObject(
		metadata,
		configuredNft.token_id,
		`NFT #${configuredNft.token_id}`,
	);
}
 ...
 */
async function loadErc1155Metadata(
	configuredNft: INftConf,
	provider: any,
	_address: string,
): Promise<INftLoadedInfo | null> {
	...
	console.log(
		`loadErc1155Metadata: URI template for token ID ${configuredNft.token_id} is ${uriTemplate}`,
	);
 ...
 */
async function loadSingleNftMetadata(
	configuredNft: INftConf,
	provider: any,
	address: string,
): Promise<INftLoadedInfo | null> {
 ...
 */
export async function loadNFTTokenMetadata(
	nftItems: INftConf[],
): Promise<void> {
	...
	const tokensToLoad = nftItems.filter(
		(nft) => !currentlyLoading.has(nft.guid) && !currentMetadata.has(nft.guid),
	);
	...
	const results = await Promise.allSettled(
		tokensToLoad.map(async (configuredNft) => {
			const metadata = await loadSingleNftMetadata(
				configuredNft,
				p,
				addr.address,
			);
...
export async function loadNFTBalances(nftItems: INftConf[]): Promise<void> {
	...
	await Promise.all(nftItems.map((nft) => refreshNftBalance(nft.guid)));
}
...
export async function loadNFTsData(nftItems: INftConf[]): Promise<void> {
	...
	const contractAddresses = [
		...new Set(nftItems.map((nft) => nft.contract_address)),
	];

	await waitForProviderReady();

	console.log(
		"Loading NFT data for",
		nftItems.length,
		"items and ",
...
async function fetchNFTMetadata(
	tokenURI: string,
): Promise<Partial<INftLoadedInfo>> {
	try {
		...
		console.debug(
			"Failed to fetch NFT metadata from",
			tokenURI,
			":",
			error instanceof Error ? error.message : error,
		);
...
function cleanupOldNftData(currentConfigs: INftConf[]) {
	...
	const currentContracts = new Set(
		currentConfigs.map((conf) => conf.contract_address),
	);
...
export async function refreshNftBalance(guid: Guid): Promise<void> {
	...
	if (!configuredNft) {
		console.debug(
			`NFT configuration not found for GUID: ${guid} (just deleted?)`,
		);
		return;
	}
	...
	if (!p || !addr) {
		console.warn(
			"Provider or address not available for refreshing NFT balance",
		);
		return;
	}
	...
	try {
		...
		console.log(
			`refreshNftBalance: Refreshing balance for NFT GUID ${guid} at contract ${configuredNft.contract_address} with token ID ${configuredNft.token_id || "N/A"}`,
		);
		...
		if (configuredNft.token_id) {
			...
			if (amount !== null) {
				console.log(
					`    Balance success: User ${addr.address} owns ${amount} of token ${configuredNft.token_id}`,
				);
			}
		}
