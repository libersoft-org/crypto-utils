...
export function formatBalance(
	balance: IBalance,
	roundToDecimals: number = -1,
	showCurrency: boolean = true,
): string | undefined {
	if (balance.amount === undefined || balance.amount === null) return undefined;
	let formatedAmount = formatUnits(
		balance.amount,
		balance.decimals !== undefined ? balance.decimals : 18,
	);
	const decimals =
		balance.decimals !== undefined && balance.decimals !== null
			? balance.decimals
			: 18;
...
export function getBalanceParts(
	b: IBalance,
	...
	try {
		const formattedRaw =
			fractionDigits !== undefined
				? formatBalance(b, fractionDigits)
				: formatBalance(b);
		...
		if (lastSpace > 0) {
			return {
				...
				symbol: formatted.slice(lastSpace + 1),
			};
		}
