...
import { refreshBalance } from "./native";
import {
	refreshTokenBalance,
	loadAllTokenInfos,
	getTokensWithContracts,
} from "./tokens";
import { loadNFTsData, nftConfs } from "./nfts";
import { fiat, updateAllFiats } from "./fiat";
	...
	await Promise.all(
		tokensWithContracts.map((token) =>
			token.contract_address
				? refreshTokenBalance(token.contract_address)
				: Promise.resolve(),
		),
	);
}
